...
enum OperatingSystem: String, CaseIterable, Sendable {
	...
	var extractorPlatform: String {
		switch self {
		case .iOS, .iPadOS: return "iPhoneOS.platform"
	...
	/// device-support folders (iPadOS shares iOS's).
	var deviceSupportDirectory: String {
		switch self {
		case .iOS, .iPadOS: return "iOS DeviceSupport"
	...
	/// Whether this platform actually serves `device`. appledb lists both iPhones
	/// and iPads under each of iOS and iPadOS (they share build numbers), but the
	/// downloadable sources are split by family: iPhones resolve only via iOS,
	/// iPads only via iPadOS. So drop the cross-listed family that has no source.
	func serves(device: String) -> Bool {
		switch self {
		case .iOS: return !device.hasPrefix("iPad")
		case .iPadOS: return device.hasPrefix("iPad")
		default: return true
		}
	}
}
...
enum AppleDB {
	/// Fetch the firmware entry for one (os, build) pair, with its full sources.
	static func firmware(os: OperatingSystem, build: String) async throws -> Firmware {
		try await get(Firmware.self, from: "https://api.appledb.dev/ios/\(os.rawValue);\(build).json")
	}
	...
	/// `firmware(os:build:)` for the chosen build to download.
	static func catalog(os: OperatingSystem) async throws -> [Release] {
		...
		struct Entry: Decodable {
			...
			var deviceMap: [String]?
			var sources: [Source]?
			// We only need to know whether a real source exists, not its shape;
			// the no-op init decodes any element so one odd row can't fail the lot.
			struct Source: Decodable { init(from decoder: Decoder) throws {} }
		}
		// appledb publishes no iPadOS catalog (that URL 404s); iPad builds live in
		// the iOS catalog (their sources are fetched per-build under the iPadOS
		// osStr). So browse iOS's list and let `serves(device:)` keep just this
		// platform's device family.
		let catalogOS = os == .iPadOS ? "iOS" : os.rawValue
		let entries = try await get([Entry].self, from: "https://api.appledb.dev/ios/\(catalogOS)/main.json")
		...
		for entry in entries {
			let deviceMap = (entry.deviceMap ?? []).filter { os.serves(device: $0) }
			guard let version = entry.version, let build = entry.build, !deviceMap.isEmpty,
				  let sources = entry.sources, !sources.isEmpty,
...
enum AEA {
	...
	static func decrypt(_ source: URL, to destination: URL, key: SymmetricKey) async throws {
		try await offMain {
			let permissions = FilePermissions(rawValue: 0o644)
			guard let input = ArchiveByteStream.fileStream(path: FilePath(source.path),
					  mode: .readOnly, options: [], permissions: permissions) else {
				throw SharedCacheError.malformedArchive("could not open \(source.lastPathComponent)")
			}
...
extension RemoteZip {
	...
	/// Stream an OTA's `payloadv2/payload.NNN` chunks straight into an
	/// AppleArchive extraction at `directory`: chunks are downloaded and
	/// pbzx-decoded in parallel and piped, in order, directly into the extractor,
	/// so the reconstructed archive is never written to disk.
	func extractOTAPayload(to directory: URL,
		...
		let names = entries.keys
			...
			.sorted()
		try SharedCacheError.throw(.malformedArchive("OTA has no payloadv2 chunks"), if: names.isEmpty)
		let chunkEntries = names.compactMap { entries[$0] }.filter { $0.compressedSize > 0 }
		try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)

		// Feed the reconstructed archive straight into the extractor through a
		// buffer pipe, so the multi-GB .yaa is never written to disk: decoded
		// chunks go into `output`, the extractor reads `input`. The pipe blocks
		// the writer when full and the reader when empty (built-in backpressure).
		...
		try await withThrowingTaskGroup(of: Void.self) { group in
			...
			// Producer: decode chunks in parallel (off the main actor) and write
			// them into the pipe strictly in order; closing `output` signals EOF.
			group.addTask {
				defer { try? output.close() }
				var pending: [Int: Data] = [:]
				var nextToWrite = 0
				try await withThrowingTaskGroup(of: (index: Int, data: Data).self) { decoders in
					var next = 0
					func enqueue() {
						let index = next, entry = chunkEntries[index]
						decoders.addTask {
							let (start, end) = try await payloadRange(of: entry)
							let raw = try await HTTP.range(url, start, end)
							// A few chunks are zip-deflated (method 8); inflate first.
							let stream = entry.method == 8 ? (try (raw as NSData).decompressed(using: .zlib) as Data) : raw
							try SharedCacheError.throw(.malformedArchive("OTA chunk \(index) is not a pbzx stream"),
													   if: !stream.starts(with: pbzxMagic))
							return (index, try PBZX.decode(stream))
						}
						next += 1
					}
					while next < min(concurrency, chunkEntries.count) { enqueue() }

					for try await result in decoders {
						pending[result.index] = result.data
						while let data = pending.removeValue(forKey: nextToWrite) {
							try data.withUnsafeBytes { raw in
								var offset = 0
								while offset < raw.count {
									offset += try output.write(from: UnsafeRawBufferPointer(rebasing: raw[offset...]))
								}
							}
							nextToWrite += 1
							progress?(nextToWrite, chunkEntries.count)
						}
						if next < chunkEntries.count { enqueue() }
					}
				}
			}
...
// MARK: - pbzx (OTA payload compression wrapper)
//
// watchOS has no IPSW; its OTA delivers the cache inside
// `payloadv2/payload.NNN` chunks. Each chunk is a pbzx stream: "pbzx" + a stream
// flags field, then repeated [block flags][block size][block], all sizes
// big-endian. Each block is either a complete xz stream or raw bytes. Decoding
// every chunk and concatenating yields one AppleArchive (.yaa).
...
nonisolated enum PBZX {
	...
	/// Decode a pbzx stream into its underlying (decompressed) bytes.
	static func decode(_ data: Data) throws -> Data {
		var reader = ByteReader(data)
		try SharedCacheError.throw(.malformedArchive("not a pbzx stream"), if: reader.bytes(4) != magic)
		reader.skip(8)                              // stream flags
		var output = Data()
		while reader.offset + 16 <= data.count {
			reader.skip(8)                          // per-block flags
			let blockSize = reader.u64BigEndian()
			guard reader.offset + blockSize <= data.count else { break }
			let block = reader.bytes(blockSize)
			if block.starts(with: xzMagic) {
				output.append(try (block as NSData).decompressed(using: .lzma) as Data)
			} else {
				output.append(block)
			}
		}
...
enum DeviceSupport {
	...
	private static func run(_ target: FirmwareTarget, root: URL, scratch: URL,
		...
		emit(.resolving)
		let firmware = try await AppleDB.firmware(os: target.os, build: target.build)
		...
		let zip = try await RemoteZip.read(link)
		let filesystem = try await filesystemRoot(zip: zip, source: source, work: work,
	...
	private static func filesystemRoot(zip: RemoteZip, source: Firmware.Source, work: URL,
		...
		if source.type == "ipsw" {
			...
			if image.name.hasSuffix(".aea") {
				...
				try await AEA.decrypt(downloaded, to: dmg, key: key)
	...
	private static func systemVersion(in filesystem: URL, zip: RemoteZip) async -> SystemVersion? {
		...
		if let entry = zip.entries["SystemVersion.plist"],
